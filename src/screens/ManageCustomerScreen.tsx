...
const ManageCustomerScreen = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter(c => c.fullName.toLowerCase().includes(q) || (c.phone || '').toLowerCase().includes(q) || (c.email || '').toLowerCase().includes(q));
  }, [items, query]);
...
const styles = StyleSheet.create({
  ...
  spentValue: { fontSize: 12, color: '#009DA5', fontWeight: 'bold' },
